...
path = Path("app/live-search/page.tsx")
text = path.read_text()

helper_marker = "export default function LiveSearchPage() {"
helpers = '''const MAX_SOURCE_PHOTO_BYTES = 20 * 1024 * 1024;
...
'''
if "preparePhotoForRecognition" not in text:
    if helper_marker not in text:
        raise SystemExit("helper marker not found")
    text = text.replace(helper_marker, helpers + helper_marker, 1)

old = '''  async function recognize(event: ChangeEvent<HTMLInputElement>) {
...
'''
new = '''  async function recognize(event: ChangeEvent<HTMLInputElement>) {
...
elif "const reader = new FileReader();" in text:
    raise SystemExit("recognize function changed unexpectedly")

text = text.replace('accept="image/jpeg,image/png,image/webp" capture="environment" onChange={recognize}', 'accept="image/*" capture="environment" onChange={recognize}', 1)

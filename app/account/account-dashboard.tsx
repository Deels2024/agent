...
type Notice = { id: number; template: string; status: string; createdAt: string; payload?: Record<string, unknown> };
...
export default function AccountDashboard({ initialName, initialEmail, initialIsAdmin }: { initialName: string; initialEmail: string; initialIsAdmin: boolean }) {
  ...
  return <main className="account-app">
    <aside className="account-sidebar">
      ...
      <nav aria-label="Разделы личного кабинета">{navigation.map((item) => <button key={item.id} className={section === item.id ? "active" : ""} onClick={() => setSection(item.id)}><span>{item.icon}</span>{item.label}{item.id === "notifications" && notices.length > 0 && <em>{Math.min(notices.length, 9)}</em>}</button>)}</nav>
      ...
      <a className="account-logout" href="/signout-with-chatgpt?return_to=/"><span>↪</span><div><b>Выйти</b><small>Завершить сеанс</small></div></a>
    </aside>
    ...
    <section className="account-main">
      ...
      <div className="account-content">
        ...
        {section === "notifications" && <section className="account-section"><div className="account-section-head"><div><span>Центр событий</span><h1>Уведомления</h1><p>Изменения цен, заказов, доставки и обращения агента.</p></div></div><article className="account-card account-notifications">{notices.length === 0 ? <Empty icon="◉" title="Новых событий нет" text="Когда что-то изменится, агент сообщит здесь." /> : notices.map((notice) => { const meta = noticeLabels[notice.template] ?? { title: humanStatus(notice.template), icon: "◉" }; return <div key={notice.id}><span>{meta.icon}</span><div><b>{meta.title}</b><p>{notice.status === "sent" ? "Доставлено" : "Сохранено в центре уведомлений"}</p></div><time>{new Date(notice.createdAt).toLocaleString("ru-RU", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })}</time></div>; })}</article></section>}

...
import SellerDeliveryPanel from "./seller-delivery-panel";

type Seller = { id: number; name: string; inn: string | null; status: string; kycStatus: string; riskScore: number } | null;
type Item = { id: number; externalId: string | null; productName: string; barcode: string | null; price: number; stock: number; weightGrams: number | null; lengthCm: number | null; widthCm: number | null; heightCm: number | null; status: string };
...
export default function SellerDashboard({ email }: { email: string }) {
  ...
  const [deliveryProfileReady, setDeliveryProfileReady] = useState(false);
  ...
  const load = useCallback(async () => {
    ...
    if (profile.seller) {
      ...
      const [itemsResponse, connectionsResponse, demandResponse, deliveryResponse] = await Promise.all([fetch("/api/sellers/inventory"), fetch("/api/sellers/connections"), fetch("/api/sellers/demand"), fetch("/api/sellers/delivery")]);
      ...
      if (deliveryResponse.ok) setDeliveryProfileReady(Boolean(((await deliveryResponse.json()) as { profile: unknown }).profile));
    }
    setMessage("");
  }, []);
  ...
  const onboardingSteps = useMemo(() => {
    const catalogReady = items.some((item) => item.status === "active" && item.stock > 0);
    const packageReady = items.some((item) => item.status === "active" && item.stock > 0 && item.weightGrams && item.lengthCm && item.widthCm && item.heightCm);
    return [
      { title: "Профиль и документы", detail: "Название, ИНН и правила продавца", done: Boolean(seller?.inn && sellerLegalComplete), href: "#seller-profile" },
      { title: "Проверка магазина", detail: "KYC и допуск администратора", done: seller?.kycStatus === "verified" && seller?.status === "active", href: "#seller-verification" },
      { title: "Товар в наличии", detail: "Активная карточка с ценой и остатком", done: catalogReady, href: "#seller-catalog" },
      { title: "Упаковка и отгрузка", detail: "Габариты товара и адрес отправления", done: Boolean(packageReady && deliveryProfileReady), href: "#seller-delivery" },
      { title: "Готовность отвечать покупателям", detail: "Магазин видит спрос и может дать цену", done: demandEligibility === "eligible" && catalogReady && deliveryProfileReady, href: "#seller-demand" },
    ];
  }, [deliveryProfileReady, demandEligibility, items, seller, sellerLegalComplete]);
  ...
  return <main className="portal-page seller-portal">
    ...
    {!message && <section className="seller-onboarding"><header><div><span className="customer-kicker">Быстрый старт</span><h2>{completedSteps === onboardingSteps.length ? "Магазин готов принимать спрос" : "Подготовьте магазин к первой продаже"}</h2><p>{completedSteps} из {onboardingSteps.length} шагов завершено. Показываем только следующий полезный шаг, без догадок.</p></div><div className="seller-onboarding-progress"><b>{Math.round((completedSteps / onboardingSteps.length) * 100)}%</b><span><i style={{ width: `${(completedSteps / onboardingSteps.length) * 100}%` }} /></span></div></header><ol>{onboardingSteps.map((step, index) => <li key={step.title} className={step.done ? "done" : nextStep?.title === step.title ? "current" : ""}><span>{step.done ? "✓" : index + 1}</span><div><b>{step.title}</b><small>{step.detail}</small></div>{!step.done && <a href={step.href}>{nextStep?.title === step.title ? "Сделать сейчас" : "Открыть"}</a>}</li>)}</ol></section>}
    <section className="portal-grid">
      <article id="seller-profile" className="portal-panel seller-profile-panel"><h2>Профиль магазина</h2><form className="portal-form" onSubmit={saveStore}><label>Название<input value={storeForm.name} onChange={(event) => setStoreForm({ ...storeForm, name: event.target.value })} placeholder="ТехноДом" required /></label><label>ИНН<input inputMode="numeric" value={storeForm.inn} onChange={(event) => setStoreForm({ ...storeForm, inn: event.target.value })} placeholder="10 или 12 цифр" /></label>{!sellerLegalComplete && <fieldset className="seller-legal-checks"><legend>Документы продавца</legend><p>Примите каждый документ отдельно. Магазин отвечает за товар, чек, гарантию и возврат.</p>{sellerRequiredDocuments.map((document) => <label key={document.slug}><input type="checkbox" checked={Boolean(sellerChecks[document.slug])} onChange={(event) => setSellerChecks((current) => ({ ...current, [document.slug]: event.target.checked }))} /><span><b>{document.shortTitle}</b><a href={`/legal/${document.slug}`} target="_blank" rel="noreferrer">Версия {document.version} ↗</a></span></label>)}</fieldset>}<button disabled={!sellerLegalComplete && !allSellerDocumentsChecked}>{seller ? "Сохранить изменения" : "Создать магазин"}</button></form>{sellerLegalComplete && <p className="seller-legal-ok">✓ Документы продавца приняты и сохранены</p>}</article>
      <article id="seller-verification" className="portal-panel"><h2>Проверка и допуск</h2><div className="verification-score"><b>{100 - (seller?.riskScore ?? 0)}</b><span>оценка доверия</span></div><ul className="seller-checklist"><li className={seller?.inn ? "done" : ""}>Реквизиты магазина</li><li className={seller?.kycStatus === "verified" ? "done" : ""}>Проверка владельца и компании</li><li className={seller?.status === "active" ? "done" : ""}>Допуск к безопасным сделкам</li></ul><p className="portal-hint">До проверки можно заполнить ассортимент, но принимать защищённые заказы нельзя.</p></article>
      {seller && <><ProductCatalog seller={seller} items={items} onReload={load} onMessage={setMessage} />
      <SellerDeliveryPanel sellerName={seller.name} onMessage={setMessage} />

...
export function platformModules(): PlatformModule[] {
  ...
  const modules: PlatformModule[] = [
    ...
    { id: 8, key: "delivery", title: "Доставка и отслеживание", description: "Адреса, точный расчёт, курьер/ПВЗ, выбор тарифа, создание отправления и трекинг работают через единый ApiShip-адаптер; доставка продавца остаётся резервом.", status: hasRuntimeValue("APISHIP_API_TOKEN") || hasRuntimeValue("DELIVERY_API_KEY") ? "ready" : "external_contract", implemented: true, missing: hasRuntimeValue("APISHIP_API_TOKEN") || hasRuntimeValue("DELIVERY_API_KEY") ? [] : ["APISHIP_API_TOKEN или личное подключение продавца"] },

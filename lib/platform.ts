...
export function platformModules(): PlatformModule[] {
  ...
  const cronReady = hasRuntimeValue("CRON_SECRET");
  const deliveryReady = hasRuntimeValue("APISHIP_API_TOKEN") || hasRuntimeValue("DELIVERY_API_KEY");
  const legalReady = runtimeValue("LEGAL_OPERATOR_REQUISITES_CONFIRMED") === "true" && runtimeValue("PRIVACY_PROCESSORS_CONFIRMED") === "true";
  const operationsReady = hasRuntimeValue("MONITORING_WEBHOOK_URL") && runtimeValue("BACKUP_POLICY_CONFIRMED") === "true";
  const modules: PlatformModule[] = [
    ...
    { id: 8, key: "delivery", title: "Доставка и отслеживание", description: "Адреса, точный расчёт, курьер/ПВЗ, выбор тарифа, создание отправления и трекинг работают через единый ApiShip-адаптер; доставка продавца остаётся резервом.", status: deliveryReady ? "ready" : "external_contract", implemented: true, missing: deliveryReady ? [] : ["APISHIP_API_TOKEN или личное подключение продавца"] },
    { id: 9, key: "notifications", title: "Email, SMS и push", description: "Очередь уведомлений хранится в базе; отправка активируется единым защищённым шлюзом и внутренним планировщиком.", status: configured(["NOTIFICATION_WEBHOOK_URL", "NOTIFICATION_WEBHOOK_SECRET"]) && cronReady ? "ready" : "needs_configuration", implemented: true, missing: [!hasRuntimeValue("NOTIFICATION_WEBHOOK_URL") ? "NOTIFICATION_WEBHOOK_URL" : "", !hasRuntimeValue("NOTIFICATION_WEBHOOK_SECRET") ? "NOTIFICATION_WEBHOOK_SECRET" : "", !cronReady ? "CRON_SECRET" : ""].filter(Boolean) },
    ...
    { id: 11, key: "price_alerts", title: "Контроль снижения цены", description: "Покупатель задаёт целевую цену; внутренний планировщик регулярно проверяет реальные предложения и создаёт уведомление при достижении цели.", status: dbReady && cronReady ? "ready" : "needs_configuration", implemented: true, missing: [!dbReady ? "DB" : "", !cronReady ? "CRON_SECRET" : ""].filter(Boolean), route: "/account" },
    ...
    { id: 13, key: "operations", title: "Мониторинг и резервирование", description: "Health-check и журнал событий готовы; боевой статус требует внешнего мониторинга и подтверждённой политики резервного копирования.", status: operationsReady ? "ready" : "needs_configuration", implemented: true, missing: [!hasRuntimeValue("MONITORING_WEBHOOK_URL") ? "MONITORING_WEBHOOK_URL" : "", runtimeValue("BACKUP_POLICY_CONFIRMED") !== "true" ? "BACKUP_POLICY_CONFIRMED=true" : ""].filter(Boolean) },
